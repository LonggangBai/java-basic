 ...
 * 总结：
     ...
     从执行结果可以看出，当客户端程序启动三个生产者、消费者线程时，生产者开始创建数据对象，
     缓冲区中数据个数为1，自增加一打印出2，传入1消费者开始进行平方处理，打印出平方结果，
     消费者线程处理完毕。依次循环操作，生产者线程关闭，直到消费者将缓冲区中的数据全部处理
     完毕时，程序运行结束。

     生产者-消费者模式能够很好的对生产者线程和消费者线程进行解耦，优化了系统结构。同时由
     于共享缓冲区的作用，允许两类线程存在执行速度上的差异，一定程度上缓解了性能瓶颈对系
     统运行的影响。
﻿﻿
 * </pre>
 * 
