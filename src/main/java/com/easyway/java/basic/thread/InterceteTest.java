 ...
 * 
 * 
 * 运行以上程序，你可以清楚地看到中断的过程。首先线程Ｂ开始运行，接着运行线程Ａ，在线程Ａ睡眠一段时间觉醒后，调用interrupt()方法中断线程Ｂ，此是可能Ｂ正在睡眠，觉醒后掏出一个InterruptedException异常，执行其中的语句，为了更清楚地看到线程的中断恢复，我在InterruptedException异常后增加了一次睡眠，当睡眠结束后，线程Ｂ调用自身的interrupt()方法恢复中断，这时测试isInterrupt()返回true,线程退出。  
