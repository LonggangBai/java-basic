 ...
 * </p>
 * 
 * @author longgangbai 2015-1-20 下午4:04:17
 */
public class CoreDataSyncImpl implements CoreDataSync {
	...
	for (String tmpTBName : coreTBNames) {
	    ...
	    // 获得核心库连接
	    DataSource coreDataSource = connectionFactory.getMasterDb();
	    Connection coreConnection = coreDataSource.getConnection();
		...
		// 3286990
		// ..
		sqlBuilder.append("Select * From ").append(tmpTBName).append(" Where id between ").append(i * ownerRecordNum + 1).append(" And ")
			.append((i * ownerRecordNum + ownerRecordNum));
		Thread workThread = new Thread(new WorkerHandler(sqlBuilder.toString(), businessType, tmpTBName));
	...
	void launchSyncData(){
	    try {
		  // 获得核心库连接
		    ...
		    Statement coreStmt = coreConnection.createStatement();
		    // 获得目标库连接
		    Connection targetConn = connectionFactory.getSalveDb().getConnection();
