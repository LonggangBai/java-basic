...
public class CollectionUtil {
	...
	public static <T> T[] reverseArray(T[] objs) {
		...
		T[] res = (T[]) java.lang.reflect.Array.newInstance(objs[0].getClass(),
				objs.length);
		// 新序号
		int k = 0;
	...
	public static <T> T[] listToArray(List<T> list) {
		...
		T[] objs = (T[]) java.lang.reflect.Array.newInstance(list.get(0)
				.getClass(), list.size());
		int i = 0; // 数组下标。
		for (T obj : list) {
			objs[i++] = obj;
	...
	public static <T> T[] addObjectToArray(T[] array, T obj) {
		...
		T[] resArray = null;
		if (isEmpty(array)) {
			resArray = (T[]) java.lang.reflect.Array.newInstance(
					obj.getClass(), 1);
			resArray[0] = obj;
			return resArray;
		}
