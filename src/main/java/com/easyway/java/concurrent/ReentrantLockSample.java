 ...
 * File Name:ReentrantLockSample.java
 * Package Name:com.easyway.java.concurrent
 * Date:2015-2-4下午4:12:06
 ...
 * ,SampleSupport2继承其并分别的具有doSomething()方法，任何进入方法的线程会运行5s左右之后counter++然后离开方法释放锁。
 * SampleSupport1是使用ReentrantLock机制，SampleSupport2是使用synchronized机制。
 * 
 * 
 * testSynchronized()和testReentrantLock()都分别开启两个线程执行测试方法executeTest()，
 * 这个方法会让一个线程先启动
 * ，另一个过100ms左右启动，并且隔1s左右试图中断后者。结果正如之前提到的第二点：interrupt()对于synchronized是没有作用的
 * ,它依然会等待5s左右获得锁执行counter
 * ++;而ReentrantLock机制可以保证在线程还未获得并且试图获得锁时如果发现线程中断，则抛出异常清除中断标记退出竞争
 * 。所以testReentrantLock()中second线程不会继续去竞争锁,执行异常内的打印语句后线程运行结束。
 * 
 * 
 * ClassName:ReentrantLockSample <br/>
 * Function: TODO ADD FUNCTION. <br/>
 * Reason: TODO ADD REASON. <br/>
 * Date: 2015-2-4 下午4:12:06 <br/>
 * 
 * @author longgangbai
 * @version
 * @since JDK 1.6
